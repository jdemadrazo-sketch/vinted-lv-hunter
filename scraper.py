...
def main():
    ...
    if not articulos:
        send("❌ No se encontraron artículos.")
        return
    ...
    gangas = sorted([a for a in articulos if a["es_muy_barato"]], key=lambda x: x["precio"])
    if gangas:
        send(f"🔥 <b>TOP {min(10, len(gangas))} GANGAS</b>")
        for g in gangas[:10]:
            papeles = "📄 Menciona original/papeles" if g["tiene_papeles"] else ""
            send(
                f"👜 <b>{g['titulo']}</b>\n"
